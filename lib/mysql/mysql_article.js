...
var list_save = function(data,callback){
  ...
  console.log(i)
  var sql = 'INSERT INTO article (portal_id, category, portal_url, title, cover, press, wr_date, scrap_date) values(?,?,?,?,?,?,?,?) ON DUPLICATE KEY UPDATE press =?';
  conn.query(sql,[i, data[i].category,data[i].portal_url,data[i].title,data[i].cover,data[i].press,data[i].date,data[i].scrapdate,data[i].press],function(err,result,fields){
    if(err){
