...
var db_reply_save = function(data,callback){
  ...
  for (k in data){
    for (i in data[k]){
      var re = data[k][i];
      conn.query(sql,[k,i,re.re_author,re.re_contents,re.re_date,re.re_reply,re.reply_likes,re.reply_hates,re.like_score,re.scrap_date],function(err,results,fields){
        if(err){
          console.log(err);
        }else{
